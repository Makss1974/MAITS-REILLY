...
def execute_reilly_engine(query: str, mode: str = "hackathon") -> str:
    ...
    # ============================================================
    # 🚀 DEMO MODE HIJACK (Fast-Track for Hackathon Presentation)
    # ============================================================
    if mode == "demo":
        logger.info("[MAIN] ⚡ ACTIVATING DEMO FAST-TRACK MODE...")
        demo_path = os.path.join(ROOT_DIR, "state", "demo_data", "mock_input.json")
        
        if not os.path.exists(demo_path):
            logger.error("[MAIN] ❌ Mock data not found at %s", demo_path)
            return "DEMO_FAILED"
            
        with open(demo_path, "r", encoding="utf-8") as f:
            inform_package = json.load(f)
        
        # Creating a dummy action program just to pass the query_id
        class MockActionProgram:
            query_id = inform_package.get("query_id", "DEMO_REQ_888")
            
        action_program = MockActionProgram()
        logger.info("[MAIN] ✅ Mock data loaded successfully. Skipping Blocks 1 & 2.")
        
    else:
        # ============================================================
        # NORMAL COMBAT MODE (Blocks 1 & 2)
        # ============================================================
        action_program = run_block_1_task(query, mode=mode)
        if hasattr(action_program, "is_rejected") and action_program.is_rejected:
            logger.warning(
                "[MAIN] ⚠️ Pipeline aborted by Block 1: %s", action_program.rejection_reason
            )
    ...
    # ============================================================
    # SHARED PIPELINE: BLOCK 4 (Analytics) & BLOCK 5 (Report)
    # ============================================================
    analytics_engine = AnalyticsEngine()
    
    # Extract signal array. Direct list for demo, dictionary extraction for normal mode
    raw_signals = inform_package.get("levels_payload", [])
    if isinstance(raw_signals, dict):
        raw_signals = [
            item for sublist in raw_signals.values() 
            for item in (sublist if isinstance(sublist, list) else [sublist])
        ]

    analytics_result = analytics_engine.run_full_analysis(action_program.query_id, raw_signals)
...
def main() -> None:
    ...
    parser.add_argument(
        "--mode", 
        choices=["hackathon", "full-tank", "private", "demo"], 
        default="hackathon"
    )
